...
def signed_point_line_errors(x_0s: list,
    ...
    ##############################
    # TODO: Student code goes here
    for x0, x1 in zip(x_0s, x_1s):
        x0 = np.asarray(x0).reshape(3,)
        x1 = np.asarray(x1).reshape(3,)

        l0 = F @ x1
        l1 = F.T @ x0

        d1 = point_line_distance(l0, x0)
        d2 = point_line_distance(l1, x1)
