...
from ps3_code import least_squares_fundamental_matrix
from ps3_code import two_view_data
from ps3_code.fundamental_matrix import (create_F, 
                                point_line_distance,
...
class TestFundamentalMatrix2(unittest.TestCase):
    ...
    def test_signed_point_line_errors(self):
        """ Check line-point errors."""
        errors = signed_point_line_errors(self.x_0s, self.F, self.x_1s)
        np.testing.assert_array_almost_equal(errors, [0.0]*18, decimal=5)
...
class TestFundamentalMatrix3(unittest.TestCase):
    ...
    def test_least_squares_optimize(self):
        ...
        F = F/10.0
        self_F = self.F/10.0
        np.testing.assert_allclose(F, self_F, rtol=1e-1)
