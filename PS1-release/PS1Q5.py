...
class Prob5():
    ...
    def my_conv2d(self, image, filter):
        ...
        ###### START CODE HERE ######
        convolved_image = np.clip(np.stack([convolve(image[..., c], filter) for c in range(3)], axis = -1), 0.0, 1.0)
        ###### END CODE HERE ######
    ...
    def prob_5_1(self):
        ...
        titles = ["Original", "Identity", "Blur", "Sobel", "Laplacian"]
        images = [self.cat, identity_image, blur_image, sobel_image, laplacian_image]
    ...
    def create_hybrid_image(self, image1, image2):
        ...
        hybrid_image = None
        ###### START CODE HERE ######
        k = self.gaussian_filter.astype(np.float64)
        # radius = (k.shape[0] - 1) // 2
        # k_cent = np.roll(np.roll(k, -radius, axis=0), -radius, axis=1)
        
        low1 = self.my_conv2d(image1, k)
        low2 = self.my_conv2d(image2, k)
        high2 = image2 - low2
        hybrid_image = np.clip(low1 + high2, 0.0, 1.0)
        ###### END CODE HERE ######
        return hybrid_image
    ...
    def prob_5_2(self):
        ...
        plt.title("Hybrid: Dog (LOW) and Cat (HIGH)")
        plt.axis("off")
        plt.show()
        
        plt.figure(figsize=(10,4))
        plt.imshow(np.clip(hybrid_image_vis, 0, 1))
        plt.title("Hybrid Image at Mult Scales")
        plt.axis("off")
        plt.show()
        ###### END CODE HERE ######
